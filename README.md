...
This repository now uses a canonical multi-module layout. Release-related modules live under `.release/`:

* `.release/core` — module POM for the library; it references the repository sources (no source files were moved).
* `.release/distribution/jitpack` — JitPack distribution module; artifactId `mvp-cimet-extraction`.
* `.release/distribution/github-packages` — GitHub Packages distribution module; artifactId `cimet-extract-lib` and distributionManagement will be configured here.

To build the full reactor and run tests locally:

```sh
mvn -B -DskipTests=false verify
```

To build only the JitPack distribution module:

```sh
mvn -f .release/distribution/jitpack/pom.xml -B package
```

Consumer coordinates (examples):

* GitHub Packages (when published): `edu.university.ecs.lab:cimet-extract-lib:1.2.1`
* JitPack (preview/stable tags): `com.github.Arid-Nova:mvp-cimet-extraction:TAG`
